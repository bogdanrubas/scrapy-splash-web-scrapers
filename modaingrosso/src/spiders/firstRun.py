...
class FirstRunSpider(scrapy.Spider):
    ...
    website = 'https://modaingrosso.pl/moje-konto/'
    ...
    with open(f'{this_dir}/splash/signIn.lua', 'r') as file:
        sign_in = file.read()
    ...
    def parse(self, response):
        ...
        categories = get_all_categories(
            self.acceptable_categories_list, response, logs)

        for log in logs.add_to_db():
            yield log

        for category in categories:
            yield form_request.executeJS(category['url'], self.get_products, self.scroll_to_bottom, self, {
    ...
    def get_products(self, response):
        ...
        for url in products:
            yield form_request.waitForSelector(url, self.getData, '.product_infos', self, cookies, {
